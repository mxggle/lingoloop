...
export const platformFetch: typeof fetch = async (input, init) => {
  ...
  const canUseDesktop = api && !(input instanceof Request) && (!init?.body || typeof init.body === "string");
  if (!canUseDesktop || !api) return globalThis.fetch(input, init);

  const result = await api.fetch(url, init);
